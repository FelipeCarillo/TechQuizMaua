...
def changeStatusFrstTime(atribute, dado):
    cursor = connection.cursor()
    use_DataBase = f"USE {DataBase}"
    cursor.execute(use_DataBase)

    code = "UPDATE usuario SET FirstLogin = 0 WHERE ({} = %(dado)s)".format(atribute)
    data = {
        'dado':dado
    }
...
def getLogin(whatAtribute, login):
    ...
    getData = "SELECT senhaUser, FirstLogin FROM usuario WHERE ({} = %(dado)s)".format(
        whatAtribute
    )
    values = {"dado": login}
