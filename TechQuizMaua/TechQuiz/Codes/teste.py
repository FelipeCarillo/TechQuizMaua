    ...
    def __init__(self, Account, Quiz):
        ...
        while i < numQuest:
            placeNumQuest = ctk.CTkButton(screen,text=i+1,font=("Roboto", 10, "bold"),width=38,height=24,fg_color=lightGray,border_color="#8F8F8F", hover_color=mainBlue,
                         border_width=5, text_color=black,corner_radius=10, command=chamaQuest(i))
            if i <=35:
            ...
            dicionarioBotoes[listaBotoes[i-1]] = i-1
        print(dicionarioBotoes[listaBotoes[0]])
...
class AvisoRespostas(ctk.CTkToplevel):
     def __init__(self):
        super().__init__()
...
class Confirmacao(ctk.CTkToplevel):
    def __init__(self):
        ...
        self.label = ctk.CTkLabel(self, text="Você deseja\nmesmo finalizar?", font=("Roboto", 30, "bold"), bg_color="#7D9EFF")
        self.label.pack(padx=20, pady=20)
        self.botaoConfirm = ctk.CTkButton(self, text="SIM", font=("Roboto", 10, "bold"), bg_color="#7D9EFF", fg_color=green,hover_color=green, command=simNao(True))
        self.botaoConfirm.place(x=40,y=250)
        self.botaoNegar = ctk.CTkButton(self, text="NÃO", font=("Roboto", 10, "bold"), bg_color="#7D9EFF", fg_color=red,hover_color=red, command=simNao(False))
        self.botaoNegar.place(x=220,y=250)
