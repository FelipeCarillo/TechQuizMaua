...
from Codigo.Images import (ImageCheckEdit, ImageRnkgLogo, ImageEdit, ImageLogoNEscrita,ImageGoBack,ImageExcel, ImageRefresh)
...
class Menu(ctk.CTkFrame):
    def __init__(self, parent, Account):
        ...
        imageRefresh = ImageRefresh([75,75])
        ...
        def botao_Confirmar():
            #Botão de Confirmar
            buttonEditar.configure(state=ctk.DISABLED)
            idUser = Account.getIdUsuario()
            Nome = inputNome.get()
            try:
                if Cargo == 3:
        ...
        if Cargo == 2:
            ...
            def atualizarComboBox():
                ListaQuizes=getAllQuizesProf(idUser)
                QuizComboBox.configure(values=ListaQuizes)
            ctk.CTkButton(gridOpcoes,command=atualizarComboBox,image=imageRefresh,width=75,height=75,bg_color=white, fg_color=white,text=None,hover=False).place(x=530,y=272.5)

            def createQuiz():
                CreateQuizScreen(Account)
            ButtonCreateQuiz = ctk.CTkButton(gridOpcoes,text="Criar",height=75,bg_color=white,fg_color=green,corner_radius=20,font=("Roboto", 30, "bold"),command=createQuiz).place(x=620,y=275)
